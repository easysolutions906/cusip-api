...
const PLANS = {
  free: { name: 'Free', validationsPerDay: 50, batchLimit: 5, ratePerMinute: 10 },
  starter: { name: 'Starter', validationsPerDay: 500, batchLimit: 25, ratePerMinute: 30 },
  pro: { name: 'Pro', validationsPerDay: 5000, batchLimit: 50, ratePerMinute: 60 },
  business: { name: 'Business', validationsPerDay: 25000, batchLimit: 100, ratePerMinute: 200 },
  enterprise: { name: 'Enterprise', validationsPerDay: 100000, batchLimit: 100, ratePerMinute: 500 },
};
...
const getUsage = (identifier) => {
  ...
  if (!current || current.date !== d) {
    usage.set(identifier, { date: d, validations: 0 });
...
const createKey = (plan = 'pro', email = null, stripeCustomerId = null) => {
  const keys = loadKeys();
  const apiKey = `cusip_${randomUUID().replace(/-/g, '')}`;
...
const authMiddleware = (req, res, next) => {
  ...
  // Daily usage check
  const u = getUsage(identifier);
  if (u.validations >= plan.validationsPerDay) {
    return res.status(429).json({
      error: 'Daily validation limit exceeded',
      used: u.validations,
      limit: plan.validationsPerDay,
      plan: keyData ? keyData.plan : 'free',
